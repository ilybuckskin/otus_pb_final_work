...
async def get_keyboard_fab(session: AsyncSession, user_id: int):
    ...
    for i in notifications:
        builder.button(text=str(i.notification_name),
                       callback_data=ManagementNotificationCallbackFactory(action="change",
                                                                 value=str(i.notification_id),
                                                                 description=i.notification_name))
    builder.button(
        text="⬅️ Отмена", callback_data=ManagementNotificationCallbackFactory(action="finish")
    )
    builder.button(
        text="❌ Отписаться", callback_data=ManagementNotificationCallbackFactory(action="unsubscribe")
    )
    builder.adjust(1)
    return builder.as_markup()


def del_or_add(user_value: dict, value: str, description: str):
...
async def update_notification_text_fab(message: types.Message, session: AsyncSession, new_value: list, user_id: int):
...
async def with_puree_fab(message: types.Message, session: AsyncSession):
    ...
    await message.answer("Вы подписаны на следующие уведомления:",
...
async def callbacks_notification_change_fab(
    ...
    if callback_data.action == "change":
        user_value = del_or_add(user_value, callback_data.value, callback_data.description)
        user_list = list(user_value.values())
        await update_notification_text_fab(callback.message, session, user_list, callback.from_user.id)

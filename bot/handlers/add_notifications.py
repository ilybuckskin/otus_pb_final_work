...
from db.requests import get_all_notifications, add_notifications
...
async def get_keyboard_fab(session: AsyncSession):
    ...
    for i in notifications:
        builder.button(text=str(i.notification_name),
                       callback_data=NotificationCallbackFactory(action="change",
                                                                 value=str(i.notification_id),
                                                                 description=i.notification_name))
    builder.button(
        text="✅ Подтвердить", callback_data=NotificationCallbackFactory(action="finish")
    )
    builder.adjust(1)
...
async def update_notification_text_fab(message: types.Message, session: AsyncSession, new_value: list):
...
async def with_puree_fab(message: types.Message, session: AsyncSession):
    ...
    await message.answer("Выберите категорию уведомлений:", reply_markup=await get_keyboard_fab(session))
...
async def callbacks_notification_change_fab(
    ...
    if callback_data.action == "change":
        ...
        user_list = list(user_value.values())
        await callback.message.edit_text(f"Успешно подписались на следующие уведомления: {user_list}")